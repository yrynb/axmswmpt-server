 ...
 * 配送员 */
@Controller
public class PeisongyuanController extends BaseController
{
    @Autowired
    private PeisongyuanMapper dao;
    @Autowired
    private PeisongyuanService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/peisongyuan_list")
    public String list()
    {
        ...
        Example example = new Example(Peisongyuan.class); //  创建一个扩展搜索类
        ...
        List<Peisongyuan> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
    ...
    public String getWhere()
    {
        ...
        @RequestMapping("/peisongyuan_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/peisongyuan_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Peisongyuan mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
    ...
    @RequestMapping("/peisongyuan_updtself")
     ...
     * @return
     */
    @RequestMapping("/peisongyuaninsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Peisongyuan post = new Peisongyuan();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
    ...
    * @return
    */
    @RequestMapping("/peisongyuanupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Peisongyuan post = new Peisongyuan();
        // 将前台表单数据填充到实体类
        if(!Request.get("gonghao").equals(""))
        post.setGonghao(Request.get("gonghao"));
        ...
        post.setXingbie(Request.get("xingbie"));
                if(!Request.get("zhaopian").equals(""))
        post.setZhaopian(Request.get("zhaopian"));
        ...
        int charuid = post.getId().intValue();
            ...
            return showSuccess("保存成功" , "peisongyuan_updtself.do");
        }
     ...
     *  后台详情
     */
    @RequestMapping("/peisongyuan_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Peisongyuan map = service.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    *  删除
    */
    @RequestMapping("/peisongyuan_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("peisongyuan").find(id);
